/* =========================================================
   Waystones - Front-end / bug-test UI build
...
const CONFIG = {
  ...
  SHEET_TITLE: "Waystones - Job Applications",
...
async function handleSaveJob(event) {
  ...
  setStatus(
    saveStatus,
    editingId ? "Updating application..." : "Saving application to Waystones..."
  );
  ...
  try {
    if (editingId) {
      ...
      if (syncResult.ok) {
        ...
        setStatus(
          saveStatus,
          "Application updated in Waystones. Google Sheets reconciliation failed.",
          "success"
        );
      ...
      if (syncResult.synced) {
        setStatus(
          saveStatus,
          "Application saved to Waystones and synced to Google Sheets.",
          "success"
        );
      } else if (syncResult.reason === "no-sheet") {
        setStatus(
          saveStatus,
          "Application saved to Waystones. Google Sheets is not configured yet.",
          "success"
        );
      } else {
        setStatus(
          saveStatus,
          "Application saved to Waystones, but Google Sheets could not sync.",
          "success"
        );
...
async function handleDeleteApplication(applicationId, button) {
  ...
  try {
    ...
    if (!syncResult.ok && state.spreadsheetId) {
      showSheetFailure(
        syncResult,
        "Application deleted in Waystones, but Google Sheets did not reconcile."
      );
    } else if (syncResult.ok) {
      setSheetStatus(
